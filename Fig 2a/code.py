...
class Algorithms:
    def __init__(self, name, function, ratios, category, alpha=1, int_range=0.2, prob=0.2):
        self.name = name
...
def drawPlot(optimal, algos, legend, title):
    ...
    for i, result in enumerate(algos):
        plt.plot(range(len(result)), result, linestyle=line_styles[i % len(line_styles)], alpha=0.5)
    plt.axhline(y=optimal, color="r", linestyle="-")
    plt.legend(legend)
    plt.title(title)
    plt.ylabel(title.split()[0])
...
def main():
    num_runs = 2000  # Number of different inputs to generate



    # Alpha algorithm list

    alg_list = [
        Algorithms("No Advice", noadvice, [], "simple"),
        Algorithms("Greedy(A)", greedy, [], "simple"),
        Algorithms("Two Competitive(A)", two_cmpi, [], "simple"),
        Algorithms("1+WR Competitive(A)", wr_cmpi, [], "simple"),
        Algorithms(
    ...
    while ii < num_runs:
        ...
        for alg in alg_list:
            if alg.category == "simple":
                ...
                rn = random.uniform(0, 1)
                delta = alg.prob  # 20% probability for A
                if rn < delta:
                    ra = advice  # Select Advice with 20% probability
                else:
                    ra = random.uniform(L, U)
    ...
    line_colors = ['blue', 'black', 'green', 'red', 'mediumorchid', 'magenta']
    ...
    # legend for alpha
    legend = ["ZCL", "PP-n", "PP-b", "PP-a", r'MIX$_{\lambda}$(50%)', "IPA(25%)" ]
