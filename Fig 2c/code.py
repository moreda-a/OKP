...
def box_plott(num_runs, alg_lists):
    ...
    custom_colors = ["blue", "green", "red", "dimgray"]
    g = sns.boxplot(
...
def cdf_plot(num_runs, alg_list):
    ...
    line_colors = ["blue", "black", "green", "red"]




    for i, alg in enumerate(alg_list):
        plt.plot(alg.ratios, cdf_base, label=alg.name, linestyle=line_styles[i], linewidth=2,color=line_colors[i])
   ...
   '''
    ...
    plt.legend(legend, loc='lower right',  fontsize=30)
    plt.show()
...
def inith():
    ...
    info_list = [
        Info(L=1, wr=0.3, n=150, capacity=1, num_runs=2000,name = '$\hat{\omega}$=0.29', alg_list=alg_list1),
        Info(L=1, wr=0.45, n=150, capacity=1, num_runs=2000, name = '$\hat{\omega}$=0.45', alg_list=copy.deepcopy(alg_list1)),
        Info(L=1, wr=0.6, n=150, capacity=1, num_runs=2000, name = '$\hat{\omega}$=0.63', alg_list=copy.deepcopy(alg_list1)),
        Info(L=1, wr=0.75, n=150, capacity=1, num_runs=2000, name = '$\hat{\omega}$=0.78', alg_list=copy.deepcopy(alg_list1)),
    ]
